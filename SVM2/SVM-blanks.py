...
def innerL(i, oS):
    ...
    if((oS.labelMat[i] * Ei < -oS.tol) and (oS.alphas[i] < oS.C)) or ((oS.labelMat[i] * Ei > oS.tol) and (oS.alphas[i] > 0)):
        ...
        if(abs(oS.alphas[j] - alphaJold) < 0.00001):
            ...
            return 0
        # 步骤6：更新alpha_i
        oS.alphas[i] += oS.labelMat[i] * \
            oS.labelMat[j] * (alphaJold - oS.alphas[j])
        # 更新Ei至误差缓存
        updateEk(oS, i)
        # 步骤7：更新b_1和b_2:
...
def testRbf(k1=1.3):
    ...
    for i in range(m):
        ...
        #######################  填空-sqx  1 #############################
        # 填空-sqx  ， 根据支持向量的点计算超平面，返回预测结果       predict
        predict = np.dot(alphas,labelMat)*kernelEval+b

        # 返回数组中各元素的正负号，用1和-1表示，并统计错误个数
        if np.sign(predict) != np.sign(labelArr[i]):
            errorCount += 1
    # 打印错误率
    print('训练集上错误率:%.2f%%' % ((float(errorCount) / m) * 100))
    # 加载测试集
    ...
    for i in range(m):
        ...
        if np.sign(predict) != np.sign(labelArr[i]):
            errorCount += 1
    # 打印错误率
    print('测试集上错误率:%.2f%%' % ((float(errorCount) / m) * 100))
